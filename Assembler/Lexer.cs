        ...
        public Token? Keyword() =>
            Current switch
            {
                ...
                _ => Lexeme.ToString() switch
                {
                    ...
                    "i16" => Transition(Start).Discard().Emit(TokenType.Integer16Type),
                    "u16" => Transition(Start).Discard().Emit(TokenType.UInteger16Type),
                    "i32" => Transition(Start).Discard().Emit(TokenType.Integer32Type),
                    "u32" => Transition(Start).Discard().Emit(TokenType.UInteger32Type),
                    "i64" => Transition(Start).Discard().Emit(TokenType.Integer64Type),
                    "u64" => Transition(Start).Discard().Emit(TokenType.UInteger64Type),
                    "f32" => Transition(Start).Discard().Emit(TokenType.Float32Type),
                    "f64" => Transition(Start).Discard().Emit(TokenType.Float64Type),
